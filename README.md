# 2026-01-otus-spring-artemyeva
<h1>Практические задания по курсу "Разработчик на Spring Framework"</h1>
<h3>Выполнила Артемьева Светлана. Группа 01-2026</h3>
<br/>
<hr/>
<h4>Список выполненных работ:</h4>
<h4>hw-01 - Домашняя работа по теме "Введение в Spring Framework"</h4>
<h4>hw-02 - Домашняя работа по теме "Конфигурирование Spring-приложений"</h4>
<h4>hw-03 - Домашняя работа по теме ""Чёрная магия" Spring Boot"</h4>
<h4>hw-04 - Домашняя работа по теме "Продвинутая конфигурация"</h4>
<h4>hw-05 - Домашняя работа по теме "DAO на Spring JDBC"</h4>
<h4>hw-06 - Домашняя работа по теме "JPQL, Spring ORM, DAO на основе Spring ORM + JPA"</h4>
<h4>hw-07 - Домашняя работа по теме ""Белая магия" Spring Data: Spring Data JPA"</h4>
<h4>hw-08 - Домашняя работа по теме "Spring Data для подключения к нереляционным БД"</h4>
<h4>hw-09 - Домашняя работа по теме "Введение в Spring MVC, Spring MVC View на Spring Boot"</h4>
<h4>hw-10 - Домашняя работа по теме "Современные приложения на Spring MVC"</h4>
<h4>hw-11 - Домашняя работа по теме "Spring WebFlux"</h4>
<h4>hw-12 - Домашняя работа по теме "Spring Security: Механизмы аутентификации"</h4>
<h4>hw-13 - Домашняя работа по теме "Spring Security: ACL"</h4>
<h4>hw-14 - Домашняя работа по теме "Spring Batch"</h4>
<h4>hw-15 - Домашняя работа по теме "Spring Integration"</h4>
<h4>hw-16 - Домашняя работа по теме "Spring Boot Actuator"</h4>
<h4>hw-17 - Домашняя работа по теме "Docker, оркестрация"</h4>